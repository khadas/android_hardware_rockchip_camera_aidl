...
ndk::ScopedAStatus ExternalCameraDevice::constructDefaultRequestSettings(
        RequestTemplate in_type,
        CameraMetadata* _aidl_return) {
    // For external camera, delegate to the session's constructDefaultRequestSettings
    std::shared_ptr<ExternalCameraDeviceSession> session = mSession.lock();
    if (session == nullptr) {
...
ndk::ScopedAStatus ExternalCameraDevice::isStreamCombinationWithSettingsSupported(
        const StreamConfiguration& in_streams, bool* _aidl_return) {
    // For external camera, delegate to the existing method
    return isStreamCombinationSupported(in_streams, _aidl_return);
}

ndk::ScopedAStatus ExternalCameraDevice::getSessionCharacteristics(
