...
#define LOG_TAG "VirtualOfflnSsn"
...
// Static instance
HandleImporter VirtualDeviceOfflineSession::sHandleImporter;

VirtualDeviceOfflineSession::VirtualDeviceOfflineSession(
...
bool VirtualDeviceOfflineSession::initialize() {
...
Status VirtualDeviceOfflineSession::importBuffer(int32_t streamId, uint64_t bufId,
                                                  buffer_handle_t buf,
...
Status VirtualDeviceOfflineSession::processCaptureResult(std::shared_ptr<HalRequest>& req) {
    ATRACE_CALL();
    // Fill output buffers
...
status_t VirtualDeviceOfflineSession::fillCaptureResult(common::V1_0::helper::CameraMetadata md,
                                                         nsecs_t timestamp) {
...
void VirtualDeviceOfflineSession::invokeProcessCaptureResultCallback(
...
Status VirtualDeviceOfflineSession::processCaptureRequestError(
...
ssize_t VirtualDeviceOfflineSession::getJpegBufferSize(int32_t, int32_t) const {
    // Empty implementation here as the jpeg buffer size is passed in by ctor
    return 0;
}

void VirtualDeviceOfflineSession::notifyError(int32_t frameNumber, int32_t streamId,
                                               ErrorCode ec) {
    NotifyMsg msg;
...
ScopedAStatus VirtualDeviceOfflineSession::setCallback(
...
void VirtualDeviceOfflineSession::initOutputThread() {
    if (mOutputThread != nullptr) {
    ...
    std::shared_ptr<VirtualDeviceOfflineSession> thiz = ref<VirtualDeviceOfflineSession>();

    mBufferRequestThread = std::make_shared<VirtualDeviceSession::BufferRequestThread>(
...
ScopedAStatus VirtualDeviceOfflineSession::getCaptureResultMetadataQueue(
...
ScopedAStatus VirtualDeviceOfflineSession::close() {
    Mutex::Autolock _il(mInterfaceLock);
    {
...
void VirtualDeviceOfflineSession::cleanupBuffersLocked(int32_t id) {
...
bool VirtualDeviceOfflineSession::OutputThread::threadLoop() {
