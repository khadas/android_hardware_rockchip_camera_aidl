...
#include "VirtualDeviceSession.h"

#include <Exif.h>
#include <VirtualDeviceOfflineSession.h>
...
// Static instances
const int VirtualDeviceSession::kMaxProcessedStream;
const int VirtualDeviceSession::kMaxStallStream;
HandleImporter VirtualDeviceSession::sHandleImporter;
...
VirtualDeviceSession::VirtualDeviceSession(
        const std::shared_ptr<ICameraDeviceCallback>& callback, const VirtualConfig& cfg,
...
void VirtualDeviceSession::createPreviewBuffer(){
    int tempWidth = mV4l2StreamingFmt.width;
    int tempHeight = mV4l2StreamingFmt.height;
...
Size VirtualDeviceSession::getMaxThumbResolution() const {
    return getMaxThumbnailResolution(mCameraCharacteristics);
}

Size VirtualDeviceSession::getMaxJpegResolution() const {
    Size ret{0, 0};
...
bool VirtualDeviceSession::initialize() {
...
bool VirtualDeviceSession::isInitFailed() {
...
void VirtualDeviceSession::initOutputThread() {
    // Grab a shared_ptr to 'this' from ndk::SharedRefBase::ref()
    std::shared_ptr<VirtualDeviceSession> thiz = ref<VirtualDeviceSession>();
    if (mSupportBufMgr) {
...
void VirtualDeviceSession::closeOutputThreadImpl() {
...
ScopedAStatus VirtualDeviceSession::configureStreams(
    ...
    sprintf(formatsource,"%c%c%c%c",format & 0xFF,
            ...
            (format >> 16) & 0xFF,
            (format >> 24) & 0xFF);
    ALOGD("CameraId:%s formatsource:%s",mCameraId.c_str(),formatsource);
...
ScopedAStatus VirtualDeviceSession::flush() {
    ATRACE_CALL();
    Mutex::Autolock _il(mInterfaceLock);
    Status status = initStatus();
    if (status != Status::OK) {
...
ScopedAStatus VirtualDeviceSession::getCaptureRequestMetadataQueue(
...
ScopedAStatus VirtualDeviceSession::getCaptureResultMetadataQueue(
        MQDescriptor<int8_t, SynchronizedReadWrite>* _aidl_return) {
    Mutex::Autolock _il(mInterfaceLock);
...
ScopedAStatus VirtualDeviceSession::isReconfigurationRequired(
...
ScopedAStatus VirtualDeviceSession::processCaptureRequest(
...
Status VirtualDeviceSession::processOneCaptureRequest(const CaptureRequest& request) {
    ...
    if (request.inputBuffer.streamId != -1) {
        ALOGE("%s: virtual does not support reprocessing!", __FUNCTION__);
...
ScopedAStatus VirtualDeviceSession::signalStreamFlush(
...
ScopedAStatus VirtualDeviceSession::switchToOffline(
...
Status VirtualDeviceSession::switchToOffline(
    ...
    std::shared_ptr<VirtualDeviceOfflineSession> sessionImpl =
            ndk::SharedRefBase::make<VirtualDeviceOfflineSession>(
...
status_t VirtualDeviceSession::initDefaultRequests() {
...
status_t VirtualDeviceSession::fillCaptureResult(common::V1_0::helper::CameraMetadata& md,
                                                        nsecs_t timestamp) {
...
int VirtualDeviceSession::configureV4l2StreamLocked(SupportedV4L2Format& v4l2Fmt,
    ...
    ALOGD("%s v4lBufferCount:%d mCfg.numVideoBuffers:%d mCfg.numStillBuffers:%d",__FUNCTION__,v4lBufferCount,mCfg.numVideoBuffers , mCfg.numStillBuffers);
...
void VirtualDeviceSession::enqueueV4l2Frame(const std::shared_ptr<V4L2Frame>& frame) {
    ATRACE_CALL();
    frame->unmap();
...
Status VirtualDeviceSession::importRequestLocked(const CaptureRequest& request,
                                                        std::vector<buffer_handle_t*>& allBufPtrs,
                                                        std::vector<int>& allFences) {
    if (mSupportBufMgr) {
...
Status VirtualDeviceSession::importRequestLockedImpl(
...
Status VirtualDeviceSession::importBuffer(int32_t streamId, uint64_t bufId,
                                                 buffer_handle_t buf,
...
Status VirtualDeviceSession::importBufferLocked(int32_t streamId, uint64_t bufId,
                                                       buffer_handle_t buf,
                                                       buffer_handle_t** outBufPtr) {
    return importBufferImpl(mCirculatingBuffers, sHandleImporter, streamId, bufId, buf, outBufPtr);
}

ScopedAStatus VirtualDeviceSession::close() {
    close(false);
    return fromStatus(Status::OK);
}

void VirtualDeviceSession::close(bool callerIsDtor) {
    Mutex::Autolock _il(mInterfaceLock);
...
bool VirtualDeviceSession::isClosed() {
    Mutex::Autolock _l(mLock);
    return mClosed;
}

ScopedAStatus VirtualDeviceSession::repeatingRequestEnd(
        int32_t /*in_frameNumber*/, const std::vector<int32_t>& /*in_streamIds*/) {
    // TODO: Figure this one out.
    return fromStatus(Status::OK);
}

int VirtualDeviceSession::v4l2StreamOffLocked() {
    if (!mV4l2Streaming) {
        return OK;
    }
...
int VirtualDeviceSession::setV4l2FpsLocked(double fps) {
    mV4l2StreamingFps = fps;
    return 0;
}

void VirtualDeviceSession::cleanupInflightFences(std::vector<int>& allFences,
                                                        size_t numFences) {
...
void VirtualDeviceSession::cleanupBuffersLocked(int id) {
...
void VirtualDeviceSession::notifyShutter(int32_t frameNumber, nsecs_t shutterTs) {
    NotifyMsg msg;
...
void VirtualDeviceSession::notifyError(int32_t frameNumber, int32_t streamId, ErrorCode ec) {
    NotifyMsg msg;
...
void VirtualDeviceSession::invokeProcessCaptureResultCallback(
...
int VirtualDeviceSession::waitForV4L2BufferReturnLocked(std::unique_lock<std::mutex>& lk) {
    ATRACE_CALL();
...
bool VirtualDeviceSession::supportOfflineLocked(int32_t streamId) {
    const Stream& stream = mStreamMap[streamId];
...
bool VirtualDeviceSession::canDropRequest(const std::vector<int32_t>& offlineStreams,
...
void VirtualDeviceSession::fillOfflineSessionInfo(
...
Status VirtualDeviceSession::isStreamCombinationSupported(
...
void VirtualDeviceSession::updateBufferCaches(
...
Status VirtualDeviceSession::processCaptureRequestError(
...
Status VirtualDeviceSession::processCaptureResult(std::shared_ptr<HalRequest>& req) {
    ATRACE_CALL();
    // Return V4L2 buffer to V4L2 buffer queue
    std::shared_ptr<V4L2Frame> v4l2Frame = std::static_pointer_cast<V4L2Frame>(req->frameIn);
...
ssize_t VirtualDeviceSession::getJpegBufferSize(int32_t width, int32_t height) const {
...
binder_status_t VirtualDeviceSession::dump(int fd, const char** /*args*/,
                                                  uint32_t /*numArgs*/) {
    bool intfLocked = tryLock(mInterfaceLock);
    if (!intfLocked) {
        dprintf(fd, "!! VirtualDeviceSession interface may be deadlocked !!\n");
    }

    if (isClosed()) {
        dprintf(fd, "virtual %s is closed\n", mCameraId.c_str());
        return STATUS_OK;
    }
    ...
    SupportedV4L2Format streamingFmt;
    {
        ...
        if (!sessionLocked) {
            dprintf(fd, "!! VirtualDeviceSession mLock may be deadlocked !!\n");
        }
    ...
    std::unordered_set<uint32_t> inflightFrames;
    {
        ...
        if (!iffLocked) {
            dprintf(fd,
                    "!! VirtualDeviceSession mInflightFramesLock may be deadlocked !!\n");
        }
    ...
    dprintf(fd, "virtual %s V4L2 FD %d, cropping type %s, %s\n", mCameraId.c_str(),
...
// Start VirtualDeviceSession::BufferRequestThread functions
VirtualDeviceSession::BufferRequestThread::BufferRequestThread(
...
int VirtualDeviceSession::BufferRequestThread::requestBufferStart(
...
int VirtualDeviceSession::BufferRequestThread::waitForBufferRequestDone(
...
Status VirtualDeviceSession::FormatConvertThread::submitRequest(
        const std::shared_ptr<HalRequest>& req) {
    std::unique_lock<std::mutex> lk(mRequestListLock);
    mRequestList.push_back(req);
    lk.unlock();
		...
		bool isDstNV21, bool is16Align, bool isYuyvFormat,int src_sw, int src_sh)
{
    ...
    ALOGE("@%s,src(%dx%d) dst fd:%d,width:%d,height:%d,isDstNV21:%d",__FUNCTION__,src_width,src_height,dst.fd,param.width,param.height,isDstNV21);
...
bool VirtualDeviceSession::FormatConvertThread::threadLoop() {
    ...
    if (req->frameIn->mFourcc == V4L2_PIX_FMT_NV12) {
        ...
        if(client.get()!= nullptr){
            ...
            frameInfo.buffer.setTo((native_handle_t*)buffer->handle);
            frameInfo.width =  req->frameIn->mWidth;
            frameInfo.height =  req->frameIn->mHeight;
            frameInfo.usage =  GRALLOC_USAGE_SW_WRITE_OFTEN | GRALLOC_USAGE_SW_READ_NEVER;
            frameInfo.frameId =  req->frameNumber;
            frameInfo.deviceId = req->cameraId;
            //ALOGV("Send frame(%dx%d) usage:%" PRIx64 ,frameInfo.width,frameInfo.height,static_cast<uint64_t>(frameInfo.usage));
            client->decoratorFrame(frameInfo,[&]( ::rockchip::hardware::hdmi::V1_0::FrameInfo _frameInfo){
                //ALOGV("[%s] Receive wrapped frame(%dx%d)",__FUNCTION__,(int)_frameInfo.width ,(int)_frameInfo.height);
...
// End VirtualDeviceSession::FormatConvertThread functions

// Start VirtualDeviceSession::OutputThread functions

VirtualDeviceSession::OutputThread::OutputThread(
...
Status VirtualDeviceSession::OutputThread::allocateIntermediateBuffers(
...
Status VirtualDeviceSession::OutputThread::submitRequest(
        const std::shared_ptr<HalRequest>& req) {
    std::unique_lock<std::mutex> lk(mRequestListLock);
    mRequestList.push_back(req);
    lk.unlock();
...
void VirtualDeviceSession::OutputThread::flush() {
    ATRACE_CALL();
    auto parent = mParent.lock();
    if (parent == nullptr) {
        ALOGE("%s: session has been disconnected!", __FUNCTION__);
...
void VirtualDeviceSession::OutputThread::dump(int fd) {
...
void VirtualDeviceSession::OutputThread::setExifMakeModel(const std::string& make,
...
VirtualDeviceSession::OutputThread::switchToOffline() {
    ATRACE_CALL();
    auto parent = mParent.lock();
    if (parent == nullptr) {
        ALOGE("%s: session has been disconnected!", __FUNCTION__);
...
int VirtualDeviceSession::OutputThread::requestBufferStart(
...
int VirtualDeviceSession::OutputThread::waitForBufferRequestDone(
...
void VirtualDeviceSession::OutputThread::signalRequestDone() {
    std::unique_lock<std::mutex> lk(mRequestListLock);
...
int VirtualDeviceSession::OutputThread::cropAndScaleLocked(
...
int VirtualDeviceSession::OutputThread::cropAndScaleThumbLocked(
        std::shared_ptr<AllocatedFrame>& in, const Size& outSz, YCbCrLayout* out) {
    Size inSz{in->mWidth, in->mHeight};
...
int VirtualDeviceSession::OutputThread::createJpegLocked(
...
void VirtualDeviceSession::OutputThread::clearIntermediateBuffers() {
    std::lock_guard<std::mutex> lk(mBufferLock);
    mYu12Frame.reset();
    mYu12ThumbFrame.reset();
...
bool VirtualDeviceSession::OutputThread::threadLoop() {
    ...
    if (req->frameIn->mFourcc != V4L2_PIX_FMT_NV24 && req->frameIn->mFourcc != V4L2_PIX_FMT_BGR24 && req->frameIn->mFourcc != V4L2_PIX_FMT_NV16 && req->frameIn->mFourcc != V4L2_PIX_FMT_NV12) {
    ...
    if (isBlobOrYv12 && (req->frameIn->mFourcc == V4L2_PIX_FMT_BGR24|| req->frameIn->mFourcc == V4L2_PIX_FMT_NV12||req->frameIn->mFourcc == V4L2_PIX_FMT_NV16)) {
