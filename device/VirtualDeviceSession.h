...
#ifndef HARDWARE_INTERFACES_CAMERA_DEVICE_DEFAULT_VIRTUALDEVICESESSION_H_
#define HARDWARE_INTERFACES_CAMERA_DEVICE_DEFAULT_VIRTUALDEVICESESSION_H_
...
class VirtualDeviceSession : public BnCameraDeviceSession, public OutputThreadInterface {
  public:
    VirtualDeviceSession(const std::shared_ptr<ICameraDeviceCallback>&,
                                const VirtualConfig& cfg,
                                const std::vector<SupportedV4L2Format>& sortedFormats,
                                const CroppingType& croppingType,
                                const common::V1_0::helper::CameraMetadata& chars,
                                const std::string& cameraId);
    ~VirtualDeviceSession() override;
    ...
    public:
        FormatConvertThread(std::weak_ptr<OutputThreadInterface> parent,std::shared_ptr<OutputThread> outputThread);
        ~FormatConvertThread();
        Status submitRequest(const std::shared_ptr<HalRequest>&);
        bool threadLoop() override;
    #ifdef RK_DEVICE
...
#endif  // HARDWARE_INTERFACES_CAMERA_DEVICE_DEFAULT_VIRTUALDEVICESESSION_H_
