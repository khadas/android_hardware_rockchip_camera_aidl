...
ndk::ScopedAStatus HdmiDevice::constructDefaultRequestSettings(
        RequestTemplate in_type,
        CameraMetadata* _aidl_return) {
    // For HDMI camera, delegate to the session's constructDefaultRequestSettings
    std::shared_ptr<HdmiDeviceSession> session = mSession.lock();
    if (session == nullptr) {
...
ndk::ScopedAStatus HdmiDevice::isStreamCombinationWithSettingsSupported(
        const StreamConfiguration& in_streams, bool* _aidl_return) {
    // For HDMI camera, delegate to the existing method
    return isStreamCombinationSupported(in_streams, _aidl_return);
}

ndk::ScopedAStatus HdmiDevice::getSessionCharacteristics(
