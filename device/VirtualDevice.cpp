...
std::string VirtualDevice::kDeviceVersion = "1.1";

VirtualDevice::VirtualDevice(const std::string& devicePath,
                                           const VirtualConfig& config)
    : mCameraId("-1"), mDevicePath(devicePath), mCfg(config) {
    std::string cameraId =
            std::to_string(mCfg.cameraIdOffset + std::atoi(devicePath.c_str()));
    mCameraId = cameraId;
...
ndk::ScopedAStatus VirtualDevice::getCameraCharacteristics(CameraMetadata* _aidl_return) {
...
ndk::ScopedAStatus VirtualDevice::getPhysicalCameraCharacteristics(const std::string&,
                                                                          CameraMetadata*) {
    ALOGE("%s: Physical camera functions are not supported for virtuals.", __FUNCTION__);
    return fromStatus(Status::ILLEGAL_ARGUMENT);
}

ndk::ScopedAStatus VirtualDevice::getResourceCost(CameraResourceCost* _aidl_return) {
    if (_aidl_return == nullptr) {
        return fromStatus(Status::ILLEGAL_ARGUMENT);
    }
...
ndk::ScopedAStatus VirtualDevice::isStreamCombinationSupported(
        const StreamConfiguration& in_streams, bool* _aidl_return) {
    if (isInitFailed()) {
        ALOGE("%s: camera %s. camera init failed!", __FUNCTION__, mCameraId.c_str());
        return fromStatus(Status::INTERNAL_ERROR);
    }
    Status s = VirtualDeviceSession::isStreamCombinationSupported(in_streams,
                                                                         mSupportedFormats, mCfg);
    *_aidl_return = s == Status::OK;
    return fromStatus(Status::OK);
}

ndk::ScopedAStatus VirtualDevice::open(
    ...
    std::shared_ptr<VirtualDeviceSession> session;
...
ndk::ScopedAStatus VirtualDevice::openInjectionSession(
...
ndk::ScopedAStatus VirtualDevice::setTorchMode(bool) {
    return fromStatus(Status::OPERATION_NOT_SUPPORTED);
}

ndk::ScopedAStatus VirtualDevice::turnOnTorchWithStrengthLevel(int32_t) {
    return fromStatus(Status::OPERATION_NOT_SUPPORTED);
}

ndk::ScopedAStatus VirtualDevice::getTorchStrengthLevel(int32_t*) {
    return fromStatus(Status::OPERATION_NOT_SUPPORTED);
}

std::shared_ptr<VirtualDeviceSession> VirtualDevice::createSession(
        const std::shared_ptr<ICameraDeviceCallback>& cb, const VirtualConfig& cfg,
    ...
    return ndk::SharedRefBase::make<VirtualDeviceSession>(
            cb, cfg, sortedFormats, croppingType, chars, cameraId);
}

bool VirtualDevice::isInitFailed() {
    Mutex::Autolock _l(mLock);
    return isInitFailedLocked();
}

bool VirtualDevice::isInitFailedLocked() {
    if (!mInitialized) {
...
void VirtualDevice::initSupportedFormatsLocked(int fd) {
...
status_t VirtualDevice::initCameraCharacteristics() {
...
status_t VirtualDevice::initAvailableCapabilities(
...
status_t VirtualDevice::initDefaultCharsKeys(
        ::android::hardware::camera::common::V1_0::helper::CameraMetadata* metadata) {
    ...
    int cameraId = std::atoi(mCameraId.c_str()- mCfg.cameraIdOffset);
    if(cameraId %2 == 0){
        facing = ANDROID_LENS_FACING_BACK;
    }
    ALOGD("cameraId:%d facing:%d",cameraId,facing);
    ...
    const int32_t requestMaxNumOutputStreams[] = {
            /*RAW*/ 0,
            /*Processed*/ VirtualDeviceSession::kMaxProcessedStream,
            /*Stall*/ VirtualDeviceSession::kMaxStallStream};
    ...
    // Orientation is a bit odd for virtual, but consider it as the orientation
    // between the virtual sensor (which is usually landscape) and the device's
...
status_t VirtualDevice::initCameraControlsCharsKeys(
...
status_t VirtualDevice::initOutputCharsKeys(
...
status_t VirtualDevice::initOutputCharsKeysByFormat(
        ::android::hardware::camera::common::V1_0::helper::CameraMetadata* metadata,
...
status_t VirtualDevice::calculateMinFps(
        ::android::hardware::camera::common::V1_0::helper::CameraMetadata* metadata) {
...
void VirtualDevice::getFrameRateList(int fd, double fpsUpperBound,
...
void VirtualDevice::updateFpsBounds(
        int fd, CroppingType cropType,
        const std::vector<VirtualConfig::FpsLimitation>& fpsLimits,
...
std::vector<SupportedV4L2Format> VirtualDevice::getCandidateSupportedFormatsLocked(
        int fd, CroppingType cropType,
        const std::vector<VirtualConfig::FpsLimitation>& fpsLimits,
        const std::vector<VirtualConfig::FpsLimitation>& depthFpsLimits,
        const Size& minStreamSize, bool depthEnabled) {
    std::vector<SupportedV4L2Format> outFmts;

    for (const auto& limit : fpsLimits) {
        if (cropType == VERTICAL) {
             SupportedV4L2Format fmt {
                .width = limit.size.width,
                .height = limit.size.height,
                .fourcc = V4L2_PIX_FMT_NV12
            };
            updateFpsBounds(fd, cropType, fpsLimits, fmt, outFmts);
        } else {  // HORIZONTAL
            SupportedV4L2Format fmt {
                .width = limit.size.height,
                .height = limit.size.width,
                .fourcc = V4L2_PIX_FMT_NV12
            };
            updateFpsBounds(fd, cropType, fpsLimits, fmt, outFmts);
        }
...
void VirtualDevice::trimSupportedFormats(CroppingType cropType,
...
binder_status_t VirtualDevice::dump(int fd, const char** args, uint32_t numArgs) {
