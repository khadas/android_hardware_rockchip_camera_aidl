...
// Static instance
HandleImporter HdmiDeviceOfflineSession::sHandleImporter;

HdmiDeviceOfflineSession::HdmiDeviceOfflineSession(
...
bool HdmiDeviceOfflineSession::initialize() {
...
Status HdmiDeviceOfflineSession::importBuffer(int32_t streamId, uint64_t bufId,
                                                  buffer_handle_t buf,
...
Status HdmiDeviceOfflineSession::processCaptureResult(std::shared_ptr<HalRequest>& req) {
    ATRACE_CALL();
    // Fill output buffers
...
status_t HdmiDeviceOfflineSession::fillCaptureResult(common::V1_0::helper::CameraMetadata md,
                                                         nsecs_t timestamp) {
...
void HdmiDeviceOfflineSession::invokeProcessCaptureResultCallback(
...
Status HdmiDeviceOfflineSession::processCaptureRequestError(
...
ssize_t HdmiDeviceOfflineSession::getJpegBufferSize(int32_t, int32_t) const {
    // Empty implementation here as the jpeg buffer size is passed in by ctor
    return 0;
}

void HdmiDeviceOfflineSession::notifyError(int32_t frameNumber, int32_t streamId,
                                               ErrorCode ec) {
    NotifyMsg msg;
...
ScopedAStatus HdmiDeviceOfflineSession::setCallback(
...
void HdmiDeviceOfflineSession::initOutputThread() {
    if (mOutputThread != nullptr) {
    ...
    std::shared_ptr<HdmiDeviceOfflineSession> thiz = ref<HdmiDeviceOfflineSession>();

    mBufferRequestThread = std::make_shared<HdmiDeviceSession::BufferRequestThread>(
...
ScopedAStatus HdmiDeviceOfflineSession::getCaptureResultMetadataQueue(
...
ScopedAStatus HdmiDeviceOfflineSession::close() {
    Mutex::Autolock _il(mInterfaceLock);
    {
...
void HdmiDeviceOfflineSession::cleanupBuffersLocked(int32_t id) {
...
bool HdmiDeviceOfflineSession::OutputThread::threadLoop() {
