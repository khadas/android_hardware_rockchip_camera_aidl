...
#include "HdmiDeviceSession.h"

#include <Exif.h>
#include <HdmiDeviceOfflineSession.h>
...
const int HdmiDeviceSession::kMaxStallStream;
HandleImporter HdmiDeviceSession::sHandleImporter;
...
HdmiDeviceSession::HdmiDeviceSession(
        const std::shared_ptr<ICameraDeviceCallback>& callback, const HdmiConfig& cfg,
...
Size HdmiDeviceSession::getMaxThumbResolution() const {
    return getMaxThumbnailResolution(mCameraCharacteristics);
}

Size HdmiDeviceSession::getMaxJpegResolution() const {
    Size ret{0, 0};
...
bool HdmiDeviceSession::initialize() {
    if (mV4l2Fd.get() < 0) {
...
bool HdmiDeviceSession::isInitFailed() {
...
void HdmiDeviceSession::initOutputThread() {
    // Grab a shared_ptr to 'this' from ndk::SharedRefBase::ref()
    std::shared_ptr<HdmiDeviceSession> thiz = ref<HdmiDeviceSession>();
    if (mSupportBufMgr) {
...
void HdmiDeviceSession::closeOutputThreadImpl() {
...
Status HdmiDeviceSession::initStatus() const {
    Mutex::Autolock _l(mLock);
...
HdmiDeviceSession::~HdmiDeviceSession() {
    if (!isClosed()) {
        ALOGE("HdmiDeviceSession deleted before close!");
        close(/*callerIsDtor*/ true);
    }
}

ScopedAStatus HdmiDeviceSession::constructDefaultRequestSettings(
...
ScopedAStatus HdmiDeviceSession::configureStreams(
...
ScopedAStatus HdmiDeviceSession::flush() {
    ATRACE_CALL();
    Mutex::Autolock _il(mInterfaceLock);
    Status status = initStatus();
    if (status != Status::OK) {
...
ScopedAStatus HdmiDeviceSession::getCaptureRequestMetadataQueue(
...
ScopedAStatus HdmiDeviceSession::getCaptureResultMetadataQueue(
        MQDescriptor<int8_t, SynchronizedReadWrite>* _aidl_return) {
    Mutex::Autolock _il(mInterfaceLock);
...
ScopedAStatus HdmiDeviceSession::isReconfigurationRequired(
...
ScopedAStatus HdmiDeviceSession::processCaptureRequest(
...
Status HdmiDeviceSession::processOneCaptureRequest(const CaptureRequest& request) {
    ATRACE_CALL();
    Status status = initStatus();
    if (status != Status::OK) {
        return status;
    ...
    if (request.inputBuffer.streamId != -1) {
        ALOGE("%s: hdmi does not support reprocessing!", __FUNCTION__);
...
ScopedAStatus HdmiDeviceSession::signalStreamFlush(
...
ScopedAStatus HdmiDeviceSession::switchToOffline(
...
Status HdmiDeviceSession::switchToOffline(
    ...
    std::shared_ptr<HdmiDeviceOfflineSession> sessionImpl =
            ndk::SharedRefBase::make<HdmiDeviceOfflineSession>(
...
status_t HdmiDeviceSession::initDefaultRequests() {
...
status_t HdmiDeviceSession::fillCaptureResult(common::V1_0::helper::CameraMetadata& md,
                                                        nsecs_t timestamp) {
...
int HdmiDeviceSession::configureV4l2StreamLocked(SupportedV4L2Format& v4l2Fmt,
                                                           double requestFps) {
    ATRACE_CALL();
    ...
    ALOGD("req_buffers.count:%d",req_buffers.count);
...
std::unique_ptr<V4L2Frame> HdmiDeviceSession::dequeueV4l2FrameLocked(nsecs_t* shutterTs) {
    ATRACE_CALL();
    int ts;
    fd_set fds;
    struct timeval tv;
...
bool HdmiDeviceSession::isSupported(
...
Status HdmiDeviceSession::importRequestLocked(const CaptureRequest& request,
                                                        std::vector<buffer_handle_t*>& allBufPtrs,
                                                        std::vector<int>& allFences) {
    if (mSupportBufMgr) {
...
Status HdmiDeviceSession::importRequestLockedImpl(
...
Status HdmiDeviceSession::importBuffer(int32_t streamId, uint64_t bufId,
                                                 buffer_handle_t buf,
...
Status HdmiDeviceSession::importBufferLocked(int32_t streamId, uint64_t bufId,
                                                       buffer_handle_t buf,
                                                       buffer_handle_t** outBufPtr) {
    return importBufferImpl(mCirculatingBuffers, sHandleImporter, streamId, bufId, buf, outBufPtr);
}

ScopedAStatus HdmiDeviceSession::close() {
    close(false);
    return fromStatus(Status::OK);
}

void HdmiDeviceSession::close(bool callerIsDtor) {
    Mutex::Autolock _il(mInterfaceLock);
...
bool HdmiDeviceSession::isClosed() {
    Mutex::Autolock _l(mLock);
    return mClosed;
}

ScopedAStatus HdmiDeviceSession::repeatingRequestEnd(
        int32_t /*in_frameNumber*/, const std::vector<int32_t>& /*in_streamIds*/) {
    // TODO: Figure this one out.
    return fromStatus(Status::OK);
}

int HdmiDeviceSession::v4l2StreamOffLocked() {
    if (!mV4l2Streaming) {
        return OK;
    }
...
int HdmiDeviceSession::setV4l2FpsLocked(double fps) {
...
void HdmiDeviceSession::cleanupInflightFences(std::vector<int>& allFences,
                                                        size_t numFences) {
...
void HdmiDeviceSession::cleanupBuffersLocked(int id) {
...
void HdmiDeviceSession::notifyShutter(int32_t frameNumber, nsecs_t shutterTs) {
    NotifyMsg msg;
...
void HdmiDeviceSession::notifyError(int32_t frameNumber, int32_t streamId, ErrorCode ec) {
    NotifyMsg msg;
...
void HdmiDeviceSession::invokeProcessCaptureResultCallback(
...
int HdmiDeviceSession::waitForV4L2BufferReturnLocked(std::unique_lock<std::mutex>& lk) {
    ATRACE_CALL();
...
bool HdmiDeviceSession::supportOfflineLocked(int32_t streamId) {
    const Stream& stream = mStreamMap[streamId];
...
bool HdmiDeviceSession::canDropRequest(const std::vector<int32_t>& offlineStreams,
...
void HdmiDeviceSession::fillOfflineSessionInfo(
...
Status HdmiDeviceSession::isStreamCombinationSupported(
...
void HdmiDeviceSession::updateBufferCaches(
...
Status HdmiDeviceSession::processCaptureRequestError(
...
Status HdmiDeviceSession::processCaptureResult(std::shared_ptr<HalRequest>& req) {
    ATRACE_CALL();
    // Return V4L2 buffer to V4L2 buffer queue
    std::shared_ptr<V4L2Frame> v4l2Frame = std::static_pointer_cast<V4L2Frame>(req->frameIn);
...
ssize_t HdmiDeviceSession::getJpegBufferSize(int32_t width, int32_t height) const {
...
binder_status_t HdmiDeviceSession::dump(int fd, const char** /*args*/,
                                                  uint32_t /*numArgs*/) {
    bool intfLocked = tryLock(mInterfaceLock);
    if (!intfLocked) {
        dprintf(fd, "!! HdmiDeviceSession interface may be deadlocked !!\n");
    }

    if (isClosed()) {
        dprintf(fd, "Hdmi %s is closed\n", mCameraId.c_str());
        return STATUS_OK;
    }
    ...
    SupportedV4L2Format streamingFmt;
    {
        ...
        if (!sessionLocked) {
            dprintf(fd, "!! HdmiDeviceSession mLock may be deadlocked !!\n");
        }
    ...
    std::unordered_set<uint32_t> inflightFrames;
    {
        ...
        if (!iffLocked) {
            dprintf(fd,
                    "!! HdmiDeviceSession mInflightFramesLock may be deadlocked !!\n");
        }
    ...
    dprintf(fd, "Hdmi %s V4L2 FD %d, cropping type %s, %s\n", mCameraId.c_str(),
...
// Start HdmiDeviceSession::BufferRequestThread functions
HdmiDeviceSession::BufferRequestThread::BufferRequestThread(
...
int HdmiDeviceSession::BufferRequestThread::requestBufferStart(
...
int HdmiDeviceSession::BufferRequestThread::waitForBufferRequestDone(
...
void HdmiDeviceSession::BufferRequestThread::waitForNextRequest() {
    ATRACE_CALL();
    std::unique_lock<std::mutex> lk(mLock);
    int waitTimes = 0;
...
bool HdmiDeviceSession::BufferRequestThread::threadLoop() {
    waitForNextRequest();
    if (exitPending()) {
        return false;
    }
...
void HdmiDeviceSession::FormatConvertThread::waitForNextRequest(std::shared_ptr<HalRequest>* out) {
    ATRACE_CALL();
    if (out == nullptr) {
        ALOGE("%s: out is null", __FUNCTION__);
        return;
...
bool HdmiDeviceSession::FormatConvertThread::threadLoop() {
    std::shared_ptr<HalRequest> req;
    auto parent = mParent.lock();
    if (parent == nullptr) {
        ALOGE("%s: session has been disconnected!", __FUNCTION__);
...
#ifdef DUMP_YUV
        {
            ...
            //if(frameCount > 5 && frameCount<10){
                ...
                sprintf(filename, "/data/camera/camera_dump_%dx%d_%d.yuv",
                        req->frameIn->mWidth,req->frameIn->mHeight,frameCount);
                fp = fopen(filename, "ab+");
                if (fp != NULL) {
                    fwrite((char*)req->mVirAddr,1,req->frameIn->mWidth*req->frameIn->mHeight*1.5,fp);
                    fclose(fp);
...
// Start HdmiDeviceSession::OutputThread functions

HdmiDeviceSession::OutputThread::OutputThread(
...
Status HdmiDeviceSession::OutputThread::allocateIntermediateBuffers(
...
Status HdmiDeviceSession::OutputThread::submitRequest(
        const std::shared_ptr<HalRequest>& req) {
    std::unique_lock<std::mutex> lk(mRequestListLock);
    mRequestList.push_back(req);
    lk.unlock();
...
void HdmiDeviceSession::OutputThread::flush() {
    ATRACE_CALL();
    auto parent = mParent.lock();
    if (parent == nullptr) {
        ALOGE("%s: session has been disconnected!", __FUNCTION__);
...
void HdmiDeviceSession::OutputThread::dump(int fd) {
...
void HdmiDeviceSession::OutputThread::setExifMakeModel(const std::string& make,
...
HdmiDeviceSession::OutputThread::switchToOffline() {
    ATRACE_CALL();
    auto parent = mParent.lock();
    if (parent == nullptr) {
        ALOGE("%s: session has been disconnected!", __FUNCTION__);
...
int HdmiDeviceSession::OutputThread::requestBufferStart(
...
int HdmiDeviceSession::OutputThread::waitForBufferRequestDone(
...
void HdmiDeviceSession::OutputThread::waitForNextRequest(
...
void HdmiDeviceSession::OutputThread::signalRequestDone() {
    std::unique_lock<std::mutex> lk(mRequestListLock);
...
int HdmiDeviceSession::OutputThread::cropAndScaleLocked(
...
int HdmiDeviceSession::OutputThread::cropAndScaleThumbLocked(
        std::shared_ptr<AllocatedFrame>& in, const Size& outSz, YCbCrLayout* out) {
    Size inSz{in->mWidth, in->mHeight};
...
int HdmiDeviceSession::OutputThread::createJpegLocked(
...
void HdmiDeviceSession::OutputThread::clearIntermediateBuffers() {
    std::lock_guard<std::mutex> lk(mBufferLock);
    mYu12Frame.reset();
    mYu12ThumbFrame.reset();
...
bool HdmiDeviceSession::OutputThread::threadLoop() {
    std::shared_ptr<HalRequest> req;
    auto parent = mParent.lock();
    if (parent == nullptr) {
        ALOGE("%s: session has been disconnected!", __FUNCTION__);
...
// End HdmiDeviceSession::OutputThread functions

#ifdef CAMERA_V3_SUPPORT
// V3 method implementations
ScopedAStatus HdmiDeviceSession::configureStreamsV2(
        const StreamConfiguration& in_requestedConfiguration,
        ::aidl::android::hardware::camera::device::ConfigureStreamsRet* _aidl_return) {
    // For HDMI camera, delegate to the existing configureStreams implementation
