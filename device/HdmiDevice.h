...
#ifndef HARDWARE_INTERFACES_CAMERA_DEVICE_DEFAULT_HDMIDEVICE_H_
#define HARDWARE_INTERFACES_CAMERA_DEVICE_DEFAULT_HDMIDEVICE_H_
...
class HdmiDevice : public BnCameraDevice {
  public:
    ...
    // camera is detached.
    HdmiDevice(const std::string& devicePath, const HdmiConfig& config);
    ~HdmiDevice() override;
  ...
  private:
    virtual std::shared_ptr<HdmiDeviceSession> createSession(
            const std::shared_ptr<ICameraDeviceCallback>&, const HdmiConfig& cfg,
    ...
    std::weak_ptr<HdmiDeviceSession> mSession =
            std::weak_ptr<HdmiDeviceSession>();
...
#endif  // HARDWARE_INTERFACES_CAMERA_DEVICE_DEFAULT_HDMIDEVICE_H_
